...
que_train, que_test, ans_train, ans_test = train_test_split(question, answer, test_size=0.1)
...
with open(data_file, 'w', encoding='utf-8') as f:
    for sent in que_train + ans_train:
        f.write(sent + '\n')
...
for Q, A in zip(que_train, ans_train):
    # Encoder 입력
...
with open('data/chatbot_train.pkl', 'wb') as f:
    pickle.dump([enc_input, dec_input, dec_output, que_test, ans_test], f, pickle.HIGHEST_PROTOCOL)
...
[(q, a) for q, a in zip(que_train[:10], ans_train[:10])]

[(q, a) for q, a in zip(que_test[:10], ans_test[:10])]
